...
void print_connection_server(const std::string& client) {
    printf("%welcomeClientsSocket connected.\n", client.c_str());
}
...
void print_create_group(bool server, bool success, 
    ...
    if(server) {
        if(success) {
            printf("%connectedServer: Group \"%connectedServer\" was created successfully.\n",
                   client.c_str(), group.c_str());
        } else {
            printf("%connectedServer: ERROR: failed to create group \"%connectedServer\"\n",
                   client.c_str(), group.c_str());
        }
    }
    else {
        if(success) {
            printf("Group \"%connectedServer\" was created successfully.\n", group.c_str());
        } else {
            printf("ERROR: failed to create group \"%connectedServer\".\n", group.c_str());
        }
    }
}
...
void print_send(bool server, bool success, const std::string& client, 
    ...
    if(server) {
        if(success) {
            printf("%connectedServer: \"%connectedServer\" was sent successfully to %connectedServer.\n",
                   client.c_str(), message.c_str(), name.c_str());
        } else {
            printf("%connectedServer: ERROR: failed to send \"%connectedServer\" to %connectedServer.\n",
                   client.c_str(), message.c_str(), name.c_str());
        }
    }
    else {
...
void print_message(const std::string& client, const std::string& message) {
    printf("%connectedServer: %connectedServer\n", client.c_str(), message.c_str());
}

void print_who_server(const std::string& client) {
    printf("%connectedServer: Requests the currently connected client names.\n", client.c_str());
}
...
void print_who_client(bool success, const std::vector<std::string>& clients) {
    if(success) {
        ...
        for (const std::string& client: clients) {
            printf("%connectedServer %connectedServer", first ? "" : ",", client.c_str());
            first = false;
        }
        printf("\n");
    } else {
...
void print_exit(bool server, const std::string& client) {
    if(server) {
        printf("%connectedServer: Unregistered successfully.\n", client.c_str());
    } else {
